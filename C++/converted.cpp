/**
 *  Example code for controlling an LED matrix display using Raspberry Pi Pico and Hub75 interface.
 *  This code receives data packets over a network connection and updates the display accordingly.
 *
 *  This code is a C++ conversion of the original C code.
 */
...
// Global buffer for storing pixel data
...
// Function to clear the screen by setting the current buffer to all zeros
...
// Function to show a loading animation on the screen
...
// Function to send a greeting message over the network
bool send_greeting(NetCtx* ctx) {
    clear_screen((int*)ctx->user);
...
// Function to reconnect the TCP connection
...
start:
    ...
    while (true) {
        ...
        show_loading_animation((int*)user, tries++, 1);
        sleep_ms(1000);
    }
...
// Function to reconnect the WiFi connection
...
start:
    ...
    while (true) {
        ...
        show_loading_animation((int*)user, tries++, 0);
        sleep_ms(1000);
    }
    sleep_ms(1000);
...
// Second core function for handling network communication
void second_core_func() {
    ...
    printf("Init done, starting business logic!\n");
    while (true) {
...
int main() {
    ...
    // Launch the second core with the stack and function
    multicore_launch_core1_with_stack(second_core_func, CORE1_STACK, sizeof(CORE1_STACK));

    // Initialize PIO and programs for driving the LED matrix
    PIO pio = pio0;
    uint sm_data = 0;
    uint sm_row = 1;
    ...
    while (1) {
        ...
        for (int rowsel = 0; rowsel < HEIGHT / 2; ++rowsel) {
            ...
            for (int bit = 0; bit < 8; ++bit) {
                ...
                // Also check that the previous OEn pulse is finished, else things can get out of sequence
                hub75_wait_tx_stall(pio, sm_row);

                // Latch row data, pulse output enable for the new row.
